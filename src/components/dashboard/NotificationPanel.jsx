...
const NotificationPanel = () => {
  return (
    <div className="space-y-6">
      ...
      {/* Today's Tasks */}
      <div className="bg-white border border-gray-150 rounded-enterprise shadow-soft p-5 transition-all duration-200 hover:shadow-premium">
        <div className="flex items-center justify-between mb-4 border-b border-gray-55 pb-2.5">
        ...
        <ul className="space-y-3" aria-label="Tasks list">
          {TASKS.map((task) => (
            <li key={task.id} className="flex items-start gap-2.5 text-xs text-gray-700 font-semibold">
              {task.done ? (
                <span className="w-4 h-4 rounded-full bg-green-50 text-feedback-success flex items-center justify-center flex-shrink-0 mt-0.5 border border-green-150">
                  <FiCheck className="w-2.5 h-2.5" />
                </span>
              ) : (
                <FiCircle className="w-4 h-4 text-gray-300 flex-shrink-0 mt-0.5" />
              )}
              <span className={task.done ? 'line-through text-gray-400 font-medium' : ''}>{task.text}</span>
            </li>
          ))}
        </ul>
      </div>
      ...
      {/* Upcoming Reminders */}
      <div className="bg-white border border-gray-150 rounded-enterprise shadow-soft p-5 transition-all duration-200 hover:shadow-premium">
        <div className="flex items-center gap-2 mb-4 border-b border-gray-55 pb-2.5">
          <FiCalendar className="w-4 h-4 text-brand-primary" />
          <h3 className="text-xs font-bold text-gray-900 uppercase tracking-wide">Upcoming Reminders</h3>
        </div>
        <div className="space-y-3.5">
          {REMINDERS.map((reminder) => (
            <div key={reminder.id} className="flex items-start gap-3">
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${
                reminder.type === 'meeting' ? 'bg-blue-50 text-blue-500' : 'bg-amber-50 text-amber-500'
              }`}>
                <FiClock className="w-4 h-4" />
              </div>
              <div>
                <p className="text-xs font-bold text-gray-800 leading-snug">{reminder.title}</p>
                <p className="text-[9px] text-gray-400 font-bold mt-1 uppercase tracking-wider">{reminder.time}</p>
              </div>
            </div>
          ))}
        </div>
          ...
          {NOTIFICATIONS.map((notif) => (
            <div key={notif.id} className="flex items-start gap-2.5">
              ...
              <div className="flex-1">
                <p className="text-xs text-gray-600 font-semibold leading-relaxed">{notif.text}</p>
                <p className="text-[9px] text-gray-400 font-bold mt-1 uppercase tracking-wider">{notif.time}</p>
              </div>
            </div>
          ))}
        </div>
