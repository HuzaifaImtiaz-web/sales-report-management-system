...
import {
  ...
  FiUploadCloud, FiDownloadCloud, FiSettings,
  FiUser, FiClipboard,
} from 'react-icons/fi';
...
const Sidebar = ({ collapsed, onToggle, mobileOpen, onMobileClose }) => {
      ...
      <div className="flex flex-col h-full relative" ref={forceExpanded ? null : containerRef}>
        ...
        <div className={`flex items-center gap-3 px-5 py-4 border-b border-white/10 ${isCollapsed ? 'justify-center px-3' : ''}`}>
          ...
          <div className={`overflow-hidden transition-all duration-300 ${isCollapsed ? 'max-w-0 opacity-0' : 'max-w-[200px] opacity-100'}`}>
            ...
            <p className="text-white/50 text-xs mt-0.5 font-medium leading-none whitespace-nowrap">Pharmaceutical</p>
          </div>
        </div>
  ...
  return (
    <>
      ...
      <aside
        className={`hidden md:flex flex-col relative bg-brand-navy flex-shrink-0 transition-all duration-300 ease-in-out h-full ${collapsed ? 'w-16' : 'w-60'
          }`}
      >
        <SidebarContent />
      </aside>
