...
import {
  ...
  FiCopy, FiFileText, FiBell, FiInfo
} from 'react-icons/fi';
...
const getTargetStats = (target) => {
  ...
  const progressPct = targetQty > 0 ? Math.min(100, Math.round((achievedQty / targetQty) * 100)) : 0;
...
/* ─── Target Form Page ───────────────────────────────────────────── */
const TargetFormPage = ({ mode, target, existingTargets, businessYearsList, onCancel, onSave }) => {
  ...
  const pageTitle = isEdit ? 'Edit Product Target' : 'Add New Target';
  const shortDesc = isEdit ? 'Modify pharmaceutical target goals.' : 'Create target objectives for pharmaceutical products.';
      ...
      <div className="space-y-6 animate-fade-in max-w-3xl mx-auto pb-12">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 dark:text-gray-500 font-medium mt-0.5">{shortDesc}</p>
          </div>
        </div>
...
/* ─── Target View Page ───────────────────────────────────────────── */
const TargetViewPage = ({ target, businessYearsList, onBack }) => {
  ...
  return (
    <DashboardLayout pageTitle="Target Details">
      <div className="space-y-6 animate-fade-in max-w-3xl mx-auto pb-12">
        <div className="flex items-center gap-4">
          <button
            onClick={onBack}
          ...
          <div>
            <h1 className="text-xl font-extrabold text-gray-900 dark:text-white tracking-tight">{stats.productName}</h1>
            <p className="text-xs text-gray-400 dark:text-gray-500 font-medium mt-0.5">{activeYearLabel}</p>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-[#0f172a] border border-gray-100 dark:border-gray-800 rounded-enterprise shadow-sm p-6 sm:p-8 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-gray-55 dark:bg-gray-800/40 p-3 rounded-lg border border-gray-100/50 dark:border-gray-800/50 col-span-2">
              <p className="text-[9px] font-bold uppercase tracking-widest text-gray-400 dark:text-gray-555">Product Name</p>
              <p className="text-xs font-bold text-gray-900 dark:text-white mt-1">{stats.productName}</p>
            </div>

            <div className="bg-gray-55 dark:bg-gray-800/40 p-3 rounded-lg border border-gray-100/50 dark:border-gray-800/50">
              <p className="text-[9px] font-bold uppercase tracking-widest text-gray-400 dark:text-gray-555">Product Group</p>
              <p className="text-xs font-semibold text-gray-800 dark:text-gray-200 mt-1">{target.group}</p>
            </div>

            <div className="bg-gray-55 dark:bg-gray-800/40 p-3 rounded-lg border border-gray-100/50 dark:border-gray-800/50">
              <p className="text-[9px] font-bold uppercase tracking-widest text-gray-400 dark:text-gray-555">Status</p>
          ...
          <div className="border border-gray-100 dark:border-gray-800 rounded-xl p-4 space-y-4">
            ...
            <div className="grid grid-cols-3 gap-2 text-center">
              ...
              <div>
                ...
                <p className="text-sm font-extrabold text-gray-500 dark:text-gray-405 mt-1">
          ...
          {target.notes && (
            <div className="border border-gray-100 dark:border-gray-800 rounded-xl p-4">
              ...
              <div className="flex items-start gap-2.5">
                <FiInfo className="w-4.5 h-4.5 text-brand-primary flex-shrink-0 mt-0.5" />
          ...
          <div className="flex items-center justify-end pt-6 border-t border-gray-100 dark:border-gray-800">
            <button
              onClick={onBack}
              className="px-5 py-2 text-xs font-bold text-gray-655 dark:text-gray-400 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg hover:border-gray-305 dark:hover:border-gray-600 hover:bg-gray-55 dark:hover:bg-gray-700 transition-all duration-150"
            >
              Back to List
            </button>
          </div>
            ...
            <div className="space-y-2.5">
              ...
              >
                <div className={`mt-0.5 w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 transition-colors
                  ${copyPrevious ? 'bg-brand-primary text-white' : 'bg-gray-150 dark:bg-gray-800 text-gray-405'}`}>
    ...
    <td colSpan={8} className="px-5 py-16 text-center">
      <div className="flex flex-col items-center gap-3">
        ...
        <div>
          ...
          <p className="text-[11px] text-gray-305 dark:text-gray-650 font-medium mt-1">Start by adding your first product target objective.</p>
        </div>
        <button
          onClick={onAdd}
...
const ProductTargets = () => {
  ...
  const currentTarget = useMemo(() => {
    if (!id) return null;
    return targets.find((t) => t.id === Number(id)) || null;
  }, [targets, id]);
  ...
  /* Handlers */
  const openAdd = () => navigate('/targets/new');
  const openEdit = (t) => navigate(`/targets/${t.id}/edit`);
  const openView = (t) => navigate(`/targets/${t.id}`);
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="relative">
              <select
                ...
                className="pl-3 pr-8 py-2.5 text-xs font-bold text-gray-700 dark:text-gray-205 bg-white dark:bg-gray-805 border border-gray-200 dark:border-gray-700 rounded-lg outline-none cursor-pointer hover:border-gray-300 dark:hover:border-gray-650 transition-colors appearance-none min-w-[210px]"
              >
                {businessYears.map((y) => (
                  <option key={y.value} value={y.value}>{y.label}</option>
                ))}
              ...
              <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-400">
                <svg className="fill-current h-4 w-4" xmlns="http://www.w3.org/2050/svg" viewBox="0 0 20 20">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2.5 text-xs font-medium text-gray-700 dark:text-gray-200 bg-gray-55 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-700 rounded-lg outline-none focus:ring-2 focus:ring-brand-primary/20 focus:border-brand-primary/40 transition-all duration-155 placeholder:text-gray-400 dark:placeholder:text-gray-505"
              />
            </div>
              ...
              {['All', 'Not Started', 'In Progress', 'Achieved'].map((s) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-md text-[11px] font-bold transition-all duration-150
                    ${statusFilter === s
                      ? 'bg-white dark:bg-gray-705 text-gray-900 dark:text-white shadow-sm border border-gray-100 dark:border-gray-600'
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-gray-800">
            <div>
              ...
              <p className="text-[10px] text-gray-400 dark:text-gray-550 font-medium mt-0.5">
                {loading ? 'Loading records...' : `Showing ${filteredWithStats.length > 0 ? startIndex + 1 : 0}–${Math.min(endIndex, filteredWithStats.length)} of ${filteredWithStats.length} objectives`}
              </p>
            </div>
          </div>
                      ...
                      <td className="px-5 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[10px] font-bold text-feedback-error bg-red-50 dark:bg-red-900/30 border border-red-100 dark:border-red-800/50 hover:bg-red-100 transition-all duration-150"
                          >
